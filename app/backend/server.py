...
async def health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warning("MongoDB validation failed (event loop may be closed): %s", e)
        db_ok = False
    except Exception as e:
        logger.warning("MongoDB validation failed: %s", e)
        db_ok = False
...
async def readiness_probe() -> Response:
    ...
    try:
        ...
        logger.warning("Readiness check failed (event loop may be closed): %s", e)
...
async def metrics() -> Response:
    ...
    prometheus_client library for more detailed metrics.
    """
    try:
        db_ok = await db.validate_connection()
        db_status = 1 if db_ok else 0
    except Exception:
        db_status = 0

    # Basic Prometheus metrics format
    metrics_output = f"""# HELP app_health Application health status (1 = healthy, 0 = unhealthy)
# TYPE app_health gauge
app_health {{service="backend"}} 1

# HELP db_connection_status Database connection status (1 = connected, 0 = disconnected)
# TYPE db_connection_status gauge
db_connection_status {{service="mongodb"}} {db_status}

# HELP app_uptime_seconds Application uptime in seconds
# TYPE app_uptime_seconds counter
app_uptime_seconds {{service="backend"}} {int((datetime.now(timezone.utc) - app_start_time).total_seconds())}
"""
...
async def shutdown_db_client() -> None:
    """Close database connection on shutdown. Safe to call after event loop closes."""
    try:
        db.disconnect()  # Ensure this call closes all async connections
    except Exception as e:
        # Don't fail if event loop is already closed
        logger.warning(f"Error during database shutdown (non-fatal): {e}")
