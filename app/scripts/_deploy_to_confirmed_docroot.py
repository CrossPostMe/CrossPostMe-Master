...
if not os.path.isdir(local_build):
    print('Local build not found', local_build); sys.exit(2)
index_path = os.path.join(local_build, 'index.html')
if not os.path.isfile(index_path):
    print('index.html missing', index_path); sys.exit(2)

ftp=ftplib.FTP(); ftp.connect(host,21,timeout=30); ftp.login(user, passwd); ftp.set_pasv(True)
# ensure public_html
try:
    ftp.cwd('public_html')
except Exception:
    try:
        ...
        print('Could not ensure public_html', e); ftp.quit(); sys.exit(1)
