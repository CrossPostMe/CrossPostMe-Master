...
"""
import ftplib, os, sys, argparse, fnmatch
...
preferred = None
for c in candidates:
    if c.endswith('public_html/static/js') or c.endswith('public_html/static/js'.lstrip('/')) or c.endswith('static/js'):
        preferred = c
        break
...
ftp.storbinary('STOR .htaccess', fp=bytes.__new__(bytes, ht))
