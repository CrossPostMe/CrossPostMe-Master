...
(async () => {
  ...
  async function checkPath(path) {
    const url = path.startsWith('http') ? path : `https://www.crosspostme.com${path}`;
    let resp = null;
    try {
    ...
    const navInfo = await page.evaluate(() => {
      const sel = document.querySelector('nav, header[role="banner"], [role="navigation"], .navbar, .site-nav');
      if (!sel) return { found: false };
      const rect = sel.getBoundingClientRect();
      const visible = rect.width > 0 && rect.height > 0;
      return { found: true, visible, text: sel.innerText ? sel.innerText.trim().slice(0,200) : '' };
    });
