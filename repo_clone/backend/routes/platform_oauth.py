...
class OAuthInitResponse(BaseModel):
    ...
    credentials_needed: Optional[List[str]] = None
    instructions: str
...
async def handle_oauth_callback(
    ...
    try:
        result = await oauth_service.handle_oauth_callback(
            platform=request.platform, code=request.code, state=request.state
        )
...
async def store_platform_credentials(
    ...
    if platform not in ["offerup", "craigslist"]:
        raise HTTPException(
            status_code=400, detail=f"Credential storage not supported for {platform}"
        )
    ...
    try:
        ...
        result = await oauth_service.store_platform_credentials(
            user_id=user_id, platform=platform, credentials=request.credentials
        )
...
async def get_connected_platforms(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch platforms: {str(e)}"
        ) from e
...
async def get_platform_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to check platform status: {str(e)}"
        ) from e
