...
from models import (Ad, AdAnalytics, AdCreate, AdUpdate, DashboardStats,
                    PostedAd)
...
async def post_ad(
    ad_id: str, platform: str, user_id: str = Depends(get_optional_current_user)
) -> PostedAd:
    ...
    try:
        ...
        if result.status == PostStatus.SUCCESS:
            ...
            elif result.status == PostStatus.LOGIN_REQUIRED:
                raise HTTPException(
                    status_code=401,
                    detail=f"Authentication failed for {platform}. Please check your credentials.",
                )
            elif result.status == PostStatus.CAPTCHA_REQUIRED:
                raise HTTPException(
                    status_code=423,
        ...
        raise HTTPException(
            status_code=500, detail=f"Internal error posting to {platform}: {str(e)}"
        ) from e
...
async def post_ad_multiple_platforms(
    ad_id: str, platforms: List[str], user_id: str = Depends(get_optional_current_user)
) -> Dict[str, Any]:
    from automation import AdData, automation_manager
    ...
    results = await automation_manager.post_to_multiple_platforms(
        list(credentials_map.keys()), ad_data, credentials_map
    )
...
async def get_posted_ads(ad_id: str) -> List[PostedAd]:
    ...
    return [PostedAd(**pa) for pa in posted_ads]


# Get All Posted Ads
@router.get("/posted/all", response_model=List[PostedAd])
async def get_all_posted_ads(platform: Optional[str] = None) -> List[PostedAd]:
    query: dict[str, Any] = {}
    if platform:
