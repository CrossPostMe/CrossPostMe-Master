...
from models import Ad, AIAdRequest, AIAdResponse, PlatformAccount, PostedAd
...
def test_platform_account_creation() -> None:
    acc = PlatformAccount(
        platform="facebook", account_name="acct", account_email="crosspostme@gmail.com"
    )
...
def test_ai_models() -> None:
    req = AIAdRequest(
        product_name="Widget", product_details="Nice", price=19.99, category="gadgets"
    )
    resp = AIAdResponse(
        title="t", description="d", suggested_categories=["a"], keywords=["k"]
    )
