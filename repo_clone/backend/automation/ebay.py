...
<{call_name}Request xmlns="urn:ebay:apis:eBLBaseComponents">
  ...
  {body_content}
    ...
    async def _make_api_call(self, xml_request: str) -> Optional[Dict]:
        """Make eBay API call"""
        try:
            headers = {
                ...
                "X-EBAY-API-CALL-NAME": self._extract_call_name(xml_request),
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    self.api_url, data=xml_request, headers=headers, timeout=30
                )
    ...
    def _parse_add_item_response(self, response: Optional[Dict]) -> PostResult:
        """Parse AddItem API response"""
        try:
            if not response:
                return PostResult(
                    status=PostStatus.FAILED, message="No response from eBay API"
                )
            ...
            if errors:
                return PostResult(
                    status=PostStatus.FAILED, message=f"eBay listing failed: {errors}"
                )

            return PostResult(
                status=PostStatus.FAILED, message="Unknown eBay API error"
            )
    ...
    async def get_categories(self) -> Optional[List[Dict]]:
        ...
        try:
            xml_request = self._build_xml_request(
                "GetCategories", "<DetailLevel>ReturnAll</DetailLevel>"
            )
    ...
    async def revise_item(self, item_id: str, ad_data: AdData) -> PostResult:
        ...
        try:
            ...
            return PostResult(
                status=PostStatus.FAILED, message="Failed to revise eBay listing"
            )
    ...
    async def end_item(self, item_id: str, reason: str = "NotAvailable") -> PostResult:
        ...
        try:
            ...
            return PostResult(
                status=PostStatus.FAILED, message="Failed to end eBay listing"
            )
