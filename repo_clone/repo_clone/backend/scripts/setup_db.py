...
async def setup_messages_indexes(db):
    ...
    await db.messages.create_index(
        [("user_id", 1), ("ad_id", 1)], name="messages_ad_idx", background=True
    )

    logger.info("Messages indexes created")


async def setup_ads_indexes(db):
    ...
    # Unique ad ID index
    await db.ads.create_index(
        [("id", 1)], name="ads_id_idx", unique=True, background=True
    )

    logger.info("Ads indexes created")


async def setup_leads_indexes(db):
    ...
    await db.leads.create_index(
        [("user_id", 1), ("ad_id", 1)], name="leads_ad_idx", background=True
    )

    logger.info("Leads indexes created")


async def setup_platform_accounts_indexes(db):
...
async def setup_secure_credentials_indexes(db):
...
async def check_existing_indexes(db):
