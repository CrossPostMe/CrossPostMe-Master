...
class EmailMonitoringService:
    ...
    def _generate_content_hash(
        self, platform: str, sender_email: str, message_text: str
    ) -> str:
        ...
        try:
            ...
            if platform:
                parsed_message = await self._parse_platform_message(
                    platform, sender, subject, body, email_message
                )
    ...
    def _identify_platform(self, sender: str, subject: str) -> Optional[str]:
        ...
        if any(pattern in sender_lower for pattern in ["craigslist.org"]):
            return "craigslist"
        elif any(pattern in sender_lower for pattern in ["facebook", "facebookmail"]):
            return "facebook"
        elif any(pattern in sender_lower for pattern in ["offerup.com"]):
            return "offerup"
        elif any(pattern in sender_lower for pattern in ["nextdoor"]):
            return "nextdoor"

        # Check subject patterns
        if any(keyword in subject_lower for keyword in ["craigslist", "cl:"]):
            return "craigslist"
        elif any(keyword in subject_lower for keyword in ["facebook", "marketplace"]):
            return "facebook"
        elif any(keyword in subject_lower for keyword in ["offerup"]):
            return "offerup"

        return None

    async def _parse_platform_message(
        self, platform: str, sender: str, subject: str, body: str, email_message
    ) -> Optional[IncomingMessageCreate]:
        ...
        try:
            ...
            message_data = {
                "platform": platform,
                "subject": subject,
                "message_text": body[:1000],  # Truncate long messages
                "source_type": "email",
                "raw_data": {
                    "sender": sender,
                    "subject": subject,
            ...
            if "original_message" in extractors:
                msg_match = re.search(
                    extractors["original_message"], body, re.DOTALL | re.IGNORECASE
                )
                if msg_match:
                    message_data["message_text"] = msg_match.group(1).strip()

            # Determine message type and priority
            message_data["message_type"] = self._classify_message_type(subject, body)
            message_data["priority"] = self._determine_priority(subject, body)

            return IncomingMessageCreate(**message_data)
    ...
    def _determine_priority(self, subject: str, body: str) -> str:
        ...
        if any(keyword in text for keyword in self.HIGH_PRIORITY_KEYWORDS):
            return "high"
        elif any(keyword in text for keyword in self.NORMAL_PRIORITY_KEYWORDS):
            return "normal"
        else:
            return "low"
    ...
    async def _store_incoming_message(self, message: IncomingMessageCreate):
        ...
        try:
            ...
            content_hash = self._generate_content_hash(
                message.platform, message.sender_email or "", message.message_text or ""
            )
            ...
            for ad in ads:
                ...
                if len(ad_words.intersection(message_words)) >= 2:
                    await db.messages.update_one(
                        {"id": message_data["id"]}, {"$set": {"ad_id": ad["id"]}}
                    )
