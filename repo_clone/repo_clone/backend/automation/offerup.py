...
class OfferUpAutomation(PlatformAutomationBase):
        ...
        try:
            ...
            try:
                await self.page.wait_for_selector(
                    '[data-testid="email-input"]', timeout=10000
                )
            except Exception:
    ...
    async def _fill_listing_form(self, ad_data: AdData) -> PostResult:
        ...
        try:
            ...
            await self.page.wait_for_selector(
                '[data-testid="sell-form"], .sell-form, #sell-form', timeout=15000
            )
    ...
    async def _select_category(self, category: str) -> bool:
        ...
        try:
            category_mapped = self.category_mapping.get(
                category.lower(), "everything-else"
            )
            ...
            for selector in category_selectors:
                if await self.page.locator(selector).is_visible(timeout=3000):
                    ...
                    try:
                        await self.page.select_option(selector, value=category_mapped)
                        ...
                        if await self.page.locator(category_option).is_visible(
                            timeout=3000
                        ):
            ...
            for selector in location_selectors:
                if await self.page.locator(selector).is_visible(timeout=3000):
                    ...
                    for suggestion_selector in suggestion_selectors:
                        if await self.page.locator(suggestion_selector).is_visible(
                            timeout=3000
                        ):
    ...
    async def _submit_listing(self) -> PostResult:
        ...
        try:
            ...
            if not submit_clicked:
                return PostResult(
                    status=PostStatus.FAILED, message="Could not find submit button"
                )
            ...
            return PostResult(
                status=PostStatus.FAILED, message="Posting status unknown"
            )
