...
class FacebookMarketplaceAutomation(PlatformAutomationBase):
    ...
    def __init__(self, headless: bool = True):
        super().__init__(
            "facebook", headless, rate_limit=0.5
    ...
    def _configure_timeouts(self) -> None:
        ...
        try:
            download_timeout = os.environ.get(
                "FACEBOOK_DOWNLOAD_TIMEOUT", str(self.default_download_timeout)
            )
        ...
        try:
            upload_timeout = os.environ.get(
                "FACEBOOK_UPLOAD_TIMEOUT", str(self.default_upload_timeout)
            )
        ...
        try:
            ...
            try:
                await self.page.wait_for_selector(
                    '[data-testid="royal_login_form"]', timeout=3000
                )
            except Exception:
                # Already logged in, check for marketplace access
                await self.page.goto(self.marketplace_url)
    ...
    async def _handle_2fa(self) -> bool:
        ...
        try:
            ...
            if await self.page.locator('[data-testid="checkpoint_title"]').is_visible(
                timeout=5000
            ):
    ...
    async def _fill_listing_form(self, ad_data: AdData) -> PostResult:
        ...
        try:
            ...
            await self.page.wait_for_selector(
                '[data-testid="marketplace-composer-form"]', timeout=10000
            )
            ...
            if await self.page.locator(category_selector).is_visible(timeout=5000):
            ...
            if not await self.safe_fill(location_selector, ad_data.location):
                return PostResult(
                    status=PostStatus.FAILED, message="Failed to fill location"
                )
            ...
            if not await self.safe_click(submit_button):
                return PostResult(
                    status=PostStatus.FAILED, message="Failed to submit listing"
                )
            ...
            for selector in success_indicators:
                if await self.page.locator(selector).is_visible(timeout=10000):
    ...
    async def _upload_images(self, image_urls: List[str]) -> bool:
        ...
        try:
            ...
            if await self.page.locator(upload_button).is_visible(timeout=5000):
                self.logger.info(
                ...
                try:
                    await self.page.set_input_files(file_input, temp_files)
                    ...
                    async with self.page.expect_file_chooser() as fc_info:
                        await self.page.click(upload_button)
    ...
    async def _download_images(
        self, image_urls: List[str], max_retries: int = 3, timeout: Optional[int] = None
    ) -> List[str]:
    ...
    async def _wait_for_upload_completion(
        ...
        try:
            ...
            while True:
                ...
                if await self.page.locator(upload_progress).count() == 0:
                    # Check if we have the expected number of uploaded images
                    uploaded_count = await self.page.locator(upload_complete).count()
    ...
    async def _extract_listing_url(self) -> Optional[str]:
        ...
        try:
            ...
            try:
                link_element = self.page.locator('a[href*="/marketplace/item/"]').first
