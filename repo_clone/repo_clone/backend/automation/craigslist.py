...
from .base import (AdData, PlatformAutomationBase, PlatformCredentials,
                   PostResult, PostStatus)
...
class CraigslistAutomation(PlatformAutomationBase):
    ...
    def __init__(self, headless: bool = True):
        super().__init__(
            "craigslist", headless, rate_limit=0.2
        ...
        try:
            ...
            try:
                await self.wait_for_selector(
                    'input[name="inputEmailHandle"]', timeout=5000
                )
            except Exception:
    ...
    async def post_ad(
        self, ad_data: AdData, credentials: PlatformCredentials
    ) -> PostResult:
        ...
        try:
            ...
            if not await self.wait_and_handle_captcha():
                return PostResult(
                    status=PostStatus.CAPTCHA_REQUIRED, message="CAPTCHA required"
                )
    ...
    async def _navigate_posting_flow(self, ad_data: AdData) -> PostResult:
        ...
        try:
            ...
            if await (await self.locator(sale_category)).is_visible(timeout=10000):
            ...
            if await (await self.locator(category_selector)).is_visible(timeout=10000):
    ...
    async def _fill_posting_form(self, ad_data: AdData) -> PostResult:
        ...
        try:
            ...
            if not await self.safe_fill(title_input, ad_data.title):
                return PostResult(
                    status=PostStatus.FAILED, message="Failed to fill title"
                )

            # Fill price
            price_input = 'input[name="price"]'
            if await (await self.locator(price_input)).is_visible():
                if not await self.safe_fill(price_input, str(int(ad_data.price))):
                    return PostResult(
                        status=PostStatus.FAILED, message="Failed to fill price"
                    )
            ...
            if not await self.safe_fill(description_textarea, ad_data.description):
                return PostResult(
                    status=PostStatus.FAILED, message="Failed to fill description"
                )
            ...
            if not await self.safe_click(continue_button):
                return PostResult(
                    status=PostStatus.FAILED, message="Failed to submit form"
                )
    ...
    async def _handle_location_selection(self, location: str) -> bool:
        ...
        try:
            ...
            if await (await self.locator(area_select)).is_visible():
                # Try to match the location to an area option
                options = await (await self.locator(f"{area_select} option")).all()
                location_lower = location.lower()
                ...
                for i, option in enumerate(options):
                    ...
                    if text:
                        ...
                        if location_lower == text_lower:
                            ...
                            break
                        elif location_lower in text_lower and matched_index == 0:
    ...
    async def _fill_contact_info(self, contact_info: Dict[str, str]) -> bool:
        ...
        try:
            ...
            if email:
                ...
                if await (await self.locator(email_input)).is_visible():
            ...
            if phone:
                ...
                if await (await self.locator(phone_input)).is_visible():
    ...
    async def _upload_images(self, image_urls: List[str]) -> bool:
        ...
        try:
            ...
            if await (await self.locator(file_input)).is_visible():
                self.logger.info(
                ...
                await (await self.locator(file_input)).set_input_files(temp_files)
    ...
    async def _download_images_for_upload(
        self, image_urls: List[str], timeout: int = 120, max_retries: int = 3
    ) -> List[str]:
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            for i, url in enumerate(image_urls):
                ...
                temp_file = os.path.join(
                    temp_dir, f"craigslist_upload_{i}{file_extension}"
                )
    ...
    async def _wait_for_craigslist_upload_completion(
        self, expected_count: int, timeout: int = 60
    ) -> bool:
        ...
        try:
            ...
            while True:
                ...
                continue_button = await (
                    await self.locator('button[type="submit"], input[value="continue"]')
                ).count()
    ...
    async def _handle_preview_and_submit(self) -> PostResult:
        ...
        try:
            ...
            for selector in publish_selectors:
                if await (await self.locator(selector)).is_visible(timeout=5000):
            ...
            for selector in success_indicators:
                if await (await self.locator(selector)).is_visible(timeout=10000):
    ...
    async def _extract_posting_url(self) -> Optional[str]:
        ...
        try:
            ...
            for pattern in link_patterns:
                link = self.page.locator(pattern).first
    ...
    async def _check_for_errors(self) -> List[str]:
        ...
        try:
            ...
            for selector in error_selectors:
                elements = await (await self.locator(selector)).all()
