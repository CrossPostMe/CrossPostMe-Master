...
class PlatformAutomationBase(ABC):
    ...
    def __init__(
        self, platform_name: str, headless: bool = True, rate_limit: float = 1.0
    ):
...
class AutomationManager:
    ...
    async def post_to_platform(
        self, platform_name: str, ad_data: AdData, credentials: PlatformCredentials
    ) -> PostResult:
        ...
        try:
            async with platform:
                ...
                if not await platform.validate_credentials(credentials):
                    return PostResult(
                        status=PostStatus.LOGIN_REQUIRED, message="Invalid credentials"
                    )
            ...
            return PostResult(
                status=PostStatus.FAILED, message=str(e), error_code="AUTOMATION_ERROR"
            )
        ...
        for platform_name in platforms:
            if platform_name in credentials_map:
                task = self.post_to_platform(
                    platform_name, ad_data, credentials_map[platform_name]
                )
        ...
        for platform_name, task in tasks:
            try:
                ...
                results[platform_name] = PostResult(
                    status=PostStatus.FAILED, message=str(e)
                )
