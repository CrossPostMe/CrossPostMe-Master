...
from auth import (ACCESS_TOKEN_EXPIRE_MINUTES, ALGORITHM,
                  REFRESH_TOKEN_EXPIRE_DAYS, SECRET_KEY, User, UserCreate,
                  UserLogin, create_access_token, create_refresh_token,
                  get_current_user_with_fallback, get_password_hash,
                  verify_password)
...
async def initialize_auth_indexes():
    ...
    try:
        ...
        logger.exception("Unexpected error while initializing auth indexes: %s", exc)
...
async def register(request: Request, user_data: UserCreate):
    ...
    # Optional early check for better UX feedback (not relied upon for correctness)
    existing_user = await db.users.find_one(
        {"$or": [{"username": user_data.username}, {"email": user_data.email}]}
    )

    if existing_user:
        # Log failed registration attempt (PII-safe: use hash instead of username/email)
    ...
    # Create new user - rely on database unique constraints for atomicity
    ...
    try:
        ...
        # Log successful registration (PII-safe: no username/email)
        logger.info(
        ...
        # Log duplicate key error (race condition, PII-safe: no username/email/client_ip)
        logger.warning(
            "Registration failed - duplicate key error | "
            f"event_type=registration_failed | "
            f"timestamp={datetime.now(timezone.utc).isoformat()} | "
    ...
    return User(
        id=user_id, username=user_data.username, email=user_data.email, is_active=True
    )
...
async def login(request: Request, login_data: UserLogin, response: Response):
    ...
    if not user_doc or not verify_password(
        login_data.password, user_doc["hashed_password"]
    ):
        # Log failed login attempt (PII-safe: use hash instead of username)
    ...
    if not user_doc.get("is_active", True):
        # Log inactive user login attempt (PII-safe: user_id only)
        logger.warning(
...
async def get_current_user_info(
    ...
    This eliminates DB queries for new tokens while maintaining compatibility with older ones.
    """
