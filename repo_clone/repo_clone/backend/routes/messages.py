...
from ..models import (IncomingMessage, IncomingMessageCreate, Lead, LeadCreate,
                      LeadUpdate, OutgoingResponse, OutgoingResponseCreate,
                      PlatformMonitoringConfig, PlatformMonitoringConfigCreate,
                      ResponseTemplate, ResponseTemplateCreate)
...
async def create_message(
    message: IncomingMessageCreate, user=Depends(get_current_user)
):
    ...
    try:
        ...
        content_hash = _generate_content_hash(
            platform, message.sender_email or "", message.message_text or ""
        )
...
async def mark_message_read(message_id: str, user=Depends(get_current_user)):
    ...
    try:
        result = await db.messages.update_one(
            {"id": message_id, "user_id": user["user_id"]}, {"$set": {"is_read": True}}
        )
...
async def update_lead(
    lead_id: str, lead_update: LeadUpdate, user=Depends(get_current_user)
):
    """Update a lead"""
    db = get_typed_db()
    ...
    try:
        ...
        if "last_contact_at" in update_data and update_data["last_contact_at"]:
            update_data["last_contact_at"] = update_data["last_contact_at"].isoformat()

        result = await db.leads.update_one(
            {"id": lead_id, "user_id": user["user_id"]}, {"$set": update_data}
        )
...
async def get_response_templates(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to fetch response templates"
        )


@router.post("/templates/", response_model=ResponseTemplate)
async def create_response_template(
    template: ResponseTemplateCreate, user=Depends(get_current_user)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve created template"
        )

    except Exception as e:
        logger.error(f"Error creating response template: {e}")
        raise HTTPException(
            status_code=500, detail="Failed to create response template"
        )
...
async def send_response(
    response: OutgoingResponseCreate, user=Depends(get_current_user)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve created response"
        )
...
async def get_monitoring_configs(user=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to fetch monitoring configs"
        )


@router.post("/monitoring/", response_model=PlatformMonitoringConfig)
async def create_monitoring_config(
    config: PlatformMonitoringConfigCreate, user=Depends(get_current_user)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to create monitoring config"
        )
