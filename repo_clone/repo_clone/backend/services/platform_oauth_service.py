...
class PlatformOAuthService:
    ...
    def _init_encryption(self) -> None:
        ...
        try:
            ...
            logger.error(f"Invalid CREDENTIAL_ENCRYPTION_KEY format or value: {str(e)}")
            raise HTTPException(
                status_code=500,
            ...
            logger.error(
                f"Failed to initialize credential encryption: {type(e).__name__}: {str(e)}"
            )
            raise HTTPException(
                status_code=500,
    ...
    async def initiate_oauth_flow(
        ...
        if platform in ["facebook", "ebay"]:
            return await self._handle_oauth_platform(
                platform, user_id, redirect_uri, config
            )

        # Handle platforms without OAuth (credential-based)
        elif platform in ["offerup", "craigslist"]:
    ...
    async def _handle_oauth_platform(
        self, platform: str, user_id: str, redirect_uri: str, config: Dict
    ) -> Dict[str, Any]:
    ...
    async def handle_oauth_callback(
        self, platform: str, code: str, state: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if platform == "facebook":
                token_data = await self._exchange_facebook_token(
                    code, oauth_state["redirect_uri"], config
                )
            elif platform == "ebay":
                ...
                raise HTTPException(
                    status_code=400, detail=f"OAuth not supported for {platform}"
                )
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to connect {platform} account: {str(e)}",
            ) from e

    async def _exchange_facebook_token(
        self, code: str, redirect_uri: str, config: Dict
    ) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            # Get user info
            user_info_url = f"{config['api_base']}/me?access_token={token_data['access_token']}&fields=id,name,email"
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.post(
                    config["token_url"], headers=headers, data=data, timeout=30.0
                )
                ...
                raise HTTPException(
                    status_code=503, detail="eBay service timeout. Please try again."
                )
            except httpx.RequestError as e:
                logger.error(f"eBay token exchange network error: {str(e)}")
                raise HTTPException(
                    status_code=503,
                ...
                logger.exception(
                    f"Unexpected error during eBay token exchange: {str(e)}"
                )
                raise HTTPException(
                    status_code=500,
    ...
    async def store_platform_credentials(
        self, user_id: str, platform: str, credentials: Dict[str, str]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to store {platform} credentials: {str(e)}",
            ) from e
