...
export default function AuthPromptModal({ open, onClose, onLogin, onRegister }){
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h2 id={titleId} className="text-xl font-semibold mb-2">Please sign in to continue</h2>
          <p id={descriptionId} className="text-sm text-gray-600 mb-4">Create an account or log in to access this feature.</p>
          <div className="flex space-x-2 justify-end">
            <button type="button" onClick={() => onCloseRef.current && onCloseRef.current()} className="px-4 py-2 border rounded">Cancel</button>
            <button type="button" onClick={onLogin} className="px-4 py-2 bg-blue-600 text-white rounded">Log in</button>
            <button type="button" onClick={onRegister} className="px-4 py-2 bg-green-600 text-white rounded">Register</button>
          </div>
        </div>
      </FocusTrap>
    </div>
